...
func TunSend(tun io.Writer)  {
	for  {
		buff := <- tunWrite
		_, err := tun.Write(buff)
		if err != nil {
...
func main()  {
	...
	var tun *multique.Tunnel
	tun, err = multique.OpenTun("eth0", ip, *ipnet, 4)
	if err != nil {
		log.Println(err.Error())
		return
	}

	for _, v := range tun.FD {
		go TunRecv(v)
		go TunSend(v)
	}
